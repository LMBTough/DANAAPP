from attacks.danaapp import DANAAPP
...
if config.attack.method == "IGAttack":
    attack = METHOD(copy.deepcopy(model),eps=EPS,alpha=ALPHA,steps=STEPS)
else:
    attack = METHOD(copy.deepcopy(model),eps=EPS,ieps=IEPS,alpha=ALPHA,steps=STEPS,method=PATH)
