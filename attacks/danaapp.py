...
import numpy as np
import copy
from torchattacks import PGD,DIFGSM,TIFGSM,MIFGSM,SINIFGSM
features = None
...
class DANAAPP(Attack):
    def __init__(self, model, eps=8/255,
                 alpha=2/255, steps=10,ieps=16/255,method="BIG"):
        super().__init__("DANAAPP", model)
        self.eps = eps
        self.alpha = alpha
        self.steps = steps
        self.ieps = ieps
    ...
    def big_method(self, model,images,y,labels,weight_np):
        fgsm = FGSM(self.ieps, 0.0, 1.0)
        for l in range(int(self.ens)):
            images_base = image_transform(images.clone())
            images_base += (torch.randn_like(images)*0.2 + 0)
            x_base = fgsm(model, images.clone(), y)
    ...
    def big_pgd_method(self, model,images,y,labels,weight_np):
        pgd = PGD(model,self.ieps, self.ieps / 10, 10)
        for l in range(int(self.ens)):
            images_base = image_transform(images.clone())
            images_base += (torch.randn_like(images)*0.2 + 0)
            x_base = pgd(images.clone(), y)
            images_base = images_base * (1 - l / self.ens) + \
                (l / self.ens) * x_base
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(images_base))
    ...
    def big_difgsm_method(self, model,images,y,labels,weight_np):
        difgsm = DIFGSM(model,self.ieps, self.ieps/10, 10)
        for l in range(int(self.ens)):
            images_base = image_transform(images.clone())
            images_base += (torch.randn_like(images)*0.2 + 0)
            x_base = difgsm(images.clone(), y)
            images_base = images_base * (1 - l / self.ens) + \
                (l / self.ens) * x_base
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(images_base))
    ...
    def big_tifgsm_method(self, model,images,y,labels,weight_np):
        tifgsm = TIFGSM(model,self.ieps, self.ieps/10, 10)
        for l in range(int(self.ens)):
            images_base = image_transform(images.clone())
            images_base += (torch.randn_like(images)*0.2 + 0)
            x_base = tifgsm(images.clone(), y)
            images_base = images_base * (1 - l / self.ens) + \
                (l / self.ens) * x_base
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(images_base))
    ...
    def big_mifgsm_method(self, model,images,y,labels,weight_np):
        mifgsm = MIFGSM(model,self.ieps, self.ieps/10, 10)
        for l in range(int(self.ens)):
            images_base = image_transform(images.clone())
            images_base += (torch.randn_like(images)*0.2 + 0)
            x_base = mifgsm(images.clone(), y)
            images_base = images_base * (1 - l / self.ens) + \
                (l / self.ens) * x_base
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(images_base))
    ...
    def big_sinifgsm_method(self, model,images,y,labels,weight_np):
        sinifgsm = SINIFGSM(model,self.ieps, self.ieps/10, 10)
        for l in range(int(self.ens)):
            images_base = image_transform(images.clone())
            images_base += (torch.randn_like(images)*0.2 + 0)
            x_base = sinifgsm(images.clone(), y)
            images_base = images_base * (1 - l / self.ens) + \
                (l / self.ens) * x_base
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(images_base))
    ...
    def pgd_method(self, model,images,y,labels,weight_np):
        images_base = image_transform(images.clone())
        images_base = images_base + torch.empty_like(images_base).uniform_(-self.ieps, self.ieps)
        images_base = torch.clamp(images_base, min=0, max=1).detach()
        for l in range(int(self.ens)):
            c = images_base + (torch.randn_like(images)*0.2 + 0)
            c = c.requires_grad_(True)
            if self.method.startswith("DIPI"):
    ...
    def difgsm_method(self, model,images,y,labels,weight_np):
        images_base = image_transform(images.clone())
        for l in range(int(self.ens)):
            c = images_base + (torch.randn_like(images)*0.2 + 0)
            c = c.requires_grad_(True)
            ...
            x_grad_real = torch.autograd.grad(
                loss, c)[0]
            x_grad_real = x_grad_real / torch.mean(torch.abs(x_grad_real), dim=(1,2,3), keepdim=True)
            images_base = images_base + 0.0025 * x_grad_real.sign()
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(c))
            else:
    ...
    def tifgsm_method(self, model,images,y,labels,weight_np):
        ...
        for l in range(int(self.ens)):
            ...
            x_grad_real = torch.autograd.grad(
                loss, c)[0]
            x_grad_real = F.conv2d(x_grad_real, stacked_kernel, stride=1, padding='same', groups=3)
            x_grad_real = x_grad_real / torch.mean(torch.abs(x_grad_real), dim=(1,2,3), keepdim=True)
            images_base = images_base + 0.0025 * x_grad_real.sign()
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(c))
    ...
    def mifgsm_method(self, model,images,y,labels,weight_np):
        images_base = image_transform(images.clone())
        ...
        for l in range(int(self.ens)):
            ...
            x_grad_real = torch.autograd.grad(
                loss, c)[0]
            x_grad_real = x_grad_real / torch.mean(torch.abs(x_grad_real), dim=(1,2,3), keepdim=True)
            x_grad_real = x_grad_real + momentum * 1
            momentum = x_grad_real
            images_base = images_base + 0.0025 * x_grad_real.sign()
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(c))
            else:
    ...
    def sini_method(self, model,images,y,labels,weight_np):
        m = 5
        images_base = image_transform(images.clone())
        momentum = torch.zeros_like(images).detach().to(self.device)
        for l in range(int(self.ens)):
            ...
            for i in torch.arange(m):
                ...
                adv_grad += torch.autograd.grad(cost, c,
                                                retain_graph=False, create_graph=False)[0]
            adv_grad = adv_grad / m
            x_grad_real = 1 * momentum + adv_grad / torch.mean(torch.abs(adv_grad), dim=(1,2,3), keepdim=True)
            momentum = x_grad_real
            images_base = images_base + 0.0025 * x_grad_real.sign()
            if self.method.startswith("DIPI"):
                logits = model(input_diversity(c))
            ...
            if iter == 0:
                if self.ens == 0:
                    ...
                    if weight_np is None:
                        ...
                        weight_np += torch.autograd.grad(
                            F.softmax(logits)*labels, features, grad_outputs=torch.ones_like(F.softmax(logits)*labels))[0]
                if self.method == "BIM" or self.method == "DIPIBIM":
                    weight_np = self.bim_method(model,images,y,labels,weight_np)
                elif self.method == "PGD" or self.method == "DIPIPGD":
                    weight_np = self.pgd_method(model,images,y,labels,weight_np)
                elif self.method == "DIFGSM" or self.method == "DIPIDIFGSM":
                    weight_np = self.difgsm_method(model,images,y,labels,weight_np)
                elif self.method == "TIFGSM" or self.method == "DIPITIFGSM":
                    weight_np = self.tifgsm_method(model,images,y,labels,weight_np)
                elif self.method == "MIFGSM" or self.method == "DIPIMIFGSM":
                    weight_np = self.mifgsm_method(model,images,y,labels,weight_np)
                elif self.method == "SINIFGSM" or self.method == "DIPISINIFGSM":
                    weight_np = self.sini_method(model,images,y,labels,weight_np)
                elif self.method == "BIG" or self.method == "DIPIBIG":
                    weight_np = self.big_method(model,images,y,labels,weight_np)
                elif self.method == "BIGPGD" or self.method == "DIPIBIGPGD":
                    weight_np = self.big_pgd_method(model,images,y,labels,weight_np)
                elif self.method == "BIGDIFGSM" or self.method == "DIPIBIGDIFGSM":
                    weight_np = self.big_difgsm_method(model,images,y,labels,weight_np)
                elif self.method == "BIGTIFGSM" or self.method == "DIPIBIGTIFGSM":
                    weight_np = self.big_tifgsm_method(model,images,y,labels,weight_np)
                elif self.method == "BIGMIFGSM" or self.method == "DIPIBIGMIFGSM":
                    weight_np = self.big_mifgsm_method(model,images,y,labels,weight_np)
                elif self.method == "BIGSINIFGSM" or self.method == "DIPIBIGSINIFGSM":
                    weight_np = self.big_sinifgsm_method(model,images,y,labels,weight_np)
